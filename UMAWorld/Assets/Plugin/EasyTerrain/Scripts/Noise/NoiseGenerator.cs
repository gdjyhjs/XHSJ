...
namespace MouseSoftware
{
	public static partial class NoiseGenerator
	{
		...
		public static NoiseSample Sum(NoiseMethod method, Vector3 point, float frequency, int octaves, float lacunarity, float persistence)
		{
			...
			sum *= (1f / range);
			sum.value = Mathf.Clamp(sum.value, -1f, 1f);
			return sum;
		}
		...
		public static NoiseSample Value2D (Vector3 point, float frequency) 
		{
			...
			sample.derivative.z = (c + d * tx) * dty;
			sample.derivative.y = 0f;
			sample.derivative *= frequency;
			return sample * (2f / hashMask) - 1f;
		}
		...
		public static NoiseSample Value3D(Vector3 point, float frequency)
		{
			...
			int ix0 = Mathf.FloorToInt(point.x);
			int iy0 = Mathf.FloorToInt(point.z);
			int iz0 = Mathf.FloorToInt(point.y);
			float tx = point.x - ix0;
			float ty = point.z - iy0;
			float tz = point.y - iz0;
			ix0 &= hashMask;
			iy0 &= hashMask;
			iz0 &= hashMask;
			int ix1 = ix0 + 1;
			...
			sample.derivative.z = (c + e * tx + (g + h * tx) * tz) * dty;
			sample.derivative.y = (d + f * tx + (g + h * tx) * ty) * dtz;
			sample.derivative *= frequency;
			return sample * (2f / hashMask) - 1f;
		}
		...
		{
			point *= frequency;
			float skew = (point.x + point.z + point.y) * (1f / 3f);
			float sx = point.x + skew;
			float sy = point.z + skew;
			float sz = point.y + skew;
			int ix = Mathf.FloorToInt(sx);
			int iy = Mathf.FloorToInt(sy);
		...
		private static NoiseSample Simplex2DPart (Vector3 point, int ix, int iy) 
		{
			...
			if (f > 0f) {
				...
				sample.derivative.z = g.y * f3 + v6f2 * y;
			}
			return sample;
		}
		...
		private static NoiseSample Simplex3DPart(Vector3 point, int ix, int iy, int iz)
		{
			...
			if (f > 0f)
			{
				...
				sample.value = v * f3;
				sample.derivative.x = g.x * f3 + v6f2 * x;
				sample.derivative.z = g.z * f3 + v6f2 * y;
				sample.derivative.y = g.y * f3 + v6f2 * z;
			}
			return sample;
		}
