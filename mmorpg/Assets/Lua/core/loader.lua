...
name:图集名字
sprite_name:小图名字
on_comp:完成回调，直接返回sprite function(sprite) end
on_err:错误回调，找不到图片
]]
function Loader:get_sprite( name, sprite_name, on_comp, on_err )
	Seven.STextureManage:getInstance():Load(name, sprite_name, on_comp, on_err)
end

--[[
GET:Loader:get_http_data(url,nil,String, on_sec,on_err)
POSE:Loader:get_http_data(url, UnityEngine.WWWForm(), String, on_sec,on_err)
]]
