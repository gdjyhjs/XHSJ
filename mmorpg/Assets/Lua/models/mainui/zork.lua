...
local Zork=class(UIBase,function(self,item_obj)
    UIBase._ctor(self, "mainui_zork_practice.u3d", item_obj) -- 资源名字全部是小写
end)
...
function Zork:on_click( obj, arg )
	...
	if cmd == "btnTenfoldTime" then -- 修炼石
		...
		Net:receive({code = ClientEnum.GUIDE_FEEBLE.ADD_MY_TIME,type = 1 },ClientProto.GuideFeebleClose)
	elseif cmd == "btnInspireDamage" then -- 鼓舞
		gf_mask_show(true)
		View("inspireCcmp",zork_practice)
		Net:receive({code = ClientEnum.GUIDE_FEEBLE.ADD_MY_HARM,type = 1 },ClientProto.GuideFeebleClose)
	elseif cmd == "btnPotionExpUp" then -- 经验药
		gf_mask_show(true)
		zork_practice:use_add_exp_gain_item()
		Net:receive({code = ClientEnum.GUIDE_FEEBLE.ADD_MY_EXP_MEDICINE,type = 1 },ClientProto.GuideFeebleClose)
	end
end
