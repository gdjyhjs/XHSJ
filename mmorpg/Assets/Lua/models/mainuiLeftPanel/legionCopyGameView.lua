...
local legionCopyGameView=class(leftPanelBase,function(self, item_obj,...)
...
function legionCopyGameView:on_asset_load(key,asset)
	...
	self:dataInit()
	legionCopyGameView._base.on_asset_load(self,key,asset)
	self:init_ui()
end

function legionCopyGameView:referInit()
	self.tween = self.refer:Get(3)
...
function legionCopyGameView:update_schedule(schedule)
	...
	local target_str = ""
	gf_print_table(data,"wtd data:"..self.copy_id)
