...
local TaskView=class(leftPanelBase,function(self, item_obj,...)
    ...
    leftPanelBase._ctor(self, "mainui_task.u3d", item_obj,...) -- 资源名字全部是小写
end)
...
function TaskView:on_asset_load(key,asset)
	...
	TaskView._base.on_asset_load(self,key,asset)
end
	...
	if sub_type == ServerEnum.TASK_SUB_TYPE.KILL_CREATURE or 
	   sub_type == ServerEnum.TASK_SUB_TYPE.COLLECT then
		for i,v in ipairs(target_list) do
			...
			if config_data then
				name = name..(config_data.name or "")..(i>1 and "," or "")
			else
...
function TaskView:on_receive( msg, id1, id2, sid )
	...
	if id1 == ClientProto.OnTouchNpcTask then -- 点击npc，有任务
		...
		-- self.battle_item:npc_lookat_character(msg.data.code)
	elseif id1 == ClientProto.OnTouchNpcNoTask then -- 点击npc，无任务
		...
		local pos = self.battle_item:get_npc_pos(msg.data.code)
		self.battle_item:find_move(pos, 
			function ()
