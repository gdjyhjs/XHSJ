...
local MagicBossView=class(leftPanelBase,function(self,item_obj,...)
	self.data = {}
	self.refresh_item_l = {}
	self.time = 1

    leftPanelBase._ctor(self, "mainui_zork_boss.u3d", item_obj,...) -- 资源名字全部是小写
end)
...
function MagicBossView:on_asset_load(key,asset)
	...
	self:send_msg()
	self:register()
	MagicBossView._base.on_asset_load(self,key,asset)
end
...
function MagicBossView:dispose()
	print("wtf MagicBossView dispose")
	self:cancel_register()
