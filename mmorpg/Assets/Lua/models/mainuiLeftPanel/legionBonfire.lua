...
local LegionBonfire=class(leftPanelBase,function(self,item_obj,...)
	print("ctor   mainui_legion_bonfire.u3d")
    leftPanelBase._ctor(self, "mainui_legion_bonfire.u3d", item_obj,...) -- 资源名字全部是小写
end)
...
function LegionBonfire:on_asset_load(key,asset)
	...
	LegionBonfire._base.on_asset_load(self,key,asset)
end
...
function LegionBonfire:on_update( dt )
	...
	if self.parent and not self.bonfire:in_activity() then
		Net:receive({},ClientProto.RefreshLeftPanel)
	end
end
