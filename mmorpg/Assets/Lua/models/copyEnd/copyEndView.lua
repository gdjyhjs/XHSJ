...
local copyEndBase = require("models.copyEnd.copyStateBase")

local CopyEndView=class(copyEndBase,function(self,item_obj,msg,copy_id)
	self.msg = msg
	self.msg.itemDrops = self:dule_msg(msg.itemDrops)
	gf_print_table(self.msg,"wtf ffself.msg:")
	self.copy_id = copy_id
    copyEndBase._ctor(self, "fuben_end.u3d", item_obj) -- 资源名字全部是小写
end)

--分离金币 经验
function CopyEndView:dule_msg(msg)
	local temp = {}
	for i,v in ipairs(msg or {}) do
		if v.code == ServerEnum.BASE_RES.COIN then
			self.coin = v.num

		elseif v.code == ServerEnum.BASE_RES.EXP then
			self.exp = v.num
		else
			table.insert(temp,v)
		end
...
function CopyEndView:get_data()
	self.exp  			= self.exp or self.msg.exp or self.msg.expDrops or 0
	self.coin 			= self.coin or self.msg.coin or self.msg.coinDrops or 0
	self.vip_add 		= gf_getItemObject("vipPrivileged"):get_battle_exp() / 100
	print("wtf self.vip_add:",self.vip_add)
	self.medicine_add 	= gf_getItemObject("buff"):get_exp_add() * 100
	print("wtf self.medicine_add:",self.medicine_add)
	...
	print("wtf exp rate1 :",self.exp,self.vip_add / 100,self.medicine_add / 100)
	self.exp = self.exp * (1 + self.vip_add / 100 + self.medicine_add / 100)
	print("wtf exp rate2 :",self.exp,self.vip_add / 100,self.medicine_add / 100)
