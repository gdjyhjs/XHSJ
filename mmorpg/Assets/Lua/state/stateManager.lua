...
StateManager =
{
    ...
    _permanent_list = {}, -- 常驻item_obj列表
    _register_item = {}, -- 所有注册并初始化的item
    _register_view_list = {}, -- 注册的view
}
...
function StateManager:get_state_transform()
...
-- 常驻列表
function StateManager:add_permanent_item( item_obj )
    self._permanent_list[item_obj] = item_obj
end

-- 获取常驻列表
function StateManager:get_permanent_items()
    return self._permanent_list
end

function StateManager:is_permanent_item( item_obj )
   return self._permanent_list[item_obj] ~= nil
end
...
-- 通过地图id，切换场景
function StateManager:change_scene( map_id )
    local config_data = ConfigMgr:get_config("mapinfo")[map_id]
    if not config_data then
        print_error("ERROR:找不到地图 id =", map_id)
        return
    end
    local stage_name = "scene"..map_id

    if not StateManager[stage_name] then
        if not LuaItemManager:get_item_obejct(stage_name) then
            local scene = LuaItemManager:register_item_object(stage_name,false,true)
            scene._require = true
        end
        local scene = LuaItemManager:get_item_obejct(stage_name)
        scene.is_loading = nil
        scene.assets = 
        {
            require("models.sceneView.sceneView")(scene, config_data)
        }
        StateManager[stage_name] = StateBase({"battle", "mainui",  "floatTextSys", stage_name})
    end
    self:set_current_state(StateManager[stage_name], true)
end
...
function StateManager:add_registerItem( item )
    self._register_item[#self._register_item+1] = item
    self._sort_register_item = true
end

function StateManager:check_register_item_sort()
    if self._is_transform then
        return
    end
    ...
    if self._sort_register_item then
        self._sort_register_item = false
        table.sort(self._register_item, sortFn)
    end
end
...
function StateManager:on_receive( msg, id1, id2, sid )

    self:check_register_item_sort() -- (修改了一下，不知道还会不会出现接收到两次的bug)

    local register_item = self._register_item

    for k,v in ipairs(register_item or {}) do
        if v.on_receive then
            v:on_receive(msg, id1, id2, sid)
        end
    end

    local stage = self:get_current_state()
    local items = stage:get_all_items()
    local register_views = self._register_view_list

    -- 显示或者隐藏当前打开ui
    if id1 == ClientProto.HideAllOpenUI then
        for k,v in ipairs(items or {}) do
            if not NotHideUI[v._key] then -- 不隐藏主界面
                for i,view in ipairs(v.assets or {}) do
        ...
        for k,view in pairs(register_views) do
            if not view.item_obj or not NotHideUI[view.item_obj._key] then
    ...
    for k,v in ipairs(items or {}) do
        for i,view in ipairs(v.assets or {}) do
            if view.on_receive and (view:is_visible() or view:is_always_receive()) then
                view:on_receive(msg, id1, id2, sid)
            end
        end
    end

    -- 分发到注册的view
    for k,v in pairs(register_views) do
        if v and (v:is_visible() or (v.is_always_receive and v:is_always_receive())) and v.on_receive then
            v:on_receive(msg, id1, id2, sid)
        end
    end
end

function StateManager:on_event( fun_name, ... )
    self:get_current_state():on_event(fun_name, ...)

    local view_list = self._register_view_list
    for k,v in pairs(view_list) do
        fn = v[fun_name]
        if v:is_visible() and fn then
            fn(v, ...)
        end
    end
end
...
-- 注册按钮事件回调
function StateManager:register_view( view )
    self._register_view_list[view] = view
end

-- 取消按钮事件回调
function StateManager:remove_register_view( view )
    self._register_view_list[view] = nil
end
