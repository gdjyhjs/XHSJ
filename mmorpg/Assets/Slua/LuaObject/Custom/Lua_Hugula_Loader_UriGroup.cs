...
public class Lua_Hugula_Loader_UriGroup : LuaObject {
	...
	static public int CheckWWWComplete_s(IntPtr l) {
		try {
			int argc = LuaDLL.lua_gettop(l);
			if(matchType(l,argc,1,typeof(Hugula.Loader.CRequest),typeof(UnityEngine.Networking.UnityWebRequest))){
				Hugula.Loader.CRequest a1;
				checkType(l,1,out a1);
			...
			else if(matchType(l,argc,1,typeof(Hugula.Loader.CRequest),typeof(UnityEngine.WWW))){
				Hugula.Loader.CRequest a1;
				checkType(l,1,out a1);
				UnityEngine.WWW a2;
				checkType(l,2,out a2);
	...
	static public int get_uriList(IntPtr l) {
		try {
			pushValue(l,true);
			pushValue(l,Hugula.Loader.UriGroup.uriList);
			return 2;
		}
		catch(Exception e) {
			return error(l,e);
	...
	static public int set_uriList(IntPtr l) {
		try {
			...
			checkType(l,2,out v);
			Hugula.Loader.UriGroup.uriList=v;
			pushValue(l,true);
			return 1;
		}
		catch(Exception e) {
