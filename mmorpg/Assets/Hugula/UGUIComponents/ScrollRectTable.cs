...
namespace Hugula.UGUIExtend
{
    ...
    public class ScrollRectTable : MonoBehaviour
    {
        ...
        /// insert data
        /// </summary>
        public static string DataInsertStr = @"return 
		function(data,index,script)
        ...
        /// </summary>
        public static string DataRemoveStr = @"return 
		function(data,index,script)
		...
		public LuaFunction onBottom;// 滑动到底端function(item,index,dataItem)

        public int pageSize = 5;
        public float renderPerFrames = 1;

        public int recordCount  // 总长度
        {
            get;
            private set;
        }
        ...
        void Render()
        {
            if (this.preRepositionIntList.Count > 0)
            {
                ...
                if (currRenderIndex + 1 <= recordCount)
                {
					...
					item.data = data [currRenderIndex + 1];
					item.index = currRenderIndex;
                    if (onItemRender != null) onItemRender.call(item, currRenderIndex + 1, data[currRenderIndex + 1]);
					if (currRenderIndex + 1 == this.recordCount) 
					{
						if (onBottom != null)
							onBottom.call (item, currRenderIndex + 1, data [currRenderIndex + 1]);
					}
                }
            }
        }
