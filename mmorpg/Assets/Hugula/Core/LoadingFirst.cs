...
public class LoadingFirst : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
		...
		#endif
		string lan = PlayerPrefs.GetString ("Language", Application.systemLanguage.ToString ());
		Debug.Log("系统当前语言："+lan);
		Seven.LanguageService ls = Seven.LanguageService.Instance;

		if (ManifestManager.CheckNeedUncompressStreamingAssets ()) {
			Debug.Log ("开始解压");
			var view = gameObject.AddComponent<UnCompressView> ();
			view.SetFinishCB (delegate {
				OnDecomper ();
			});
			StartCoroutine(
				ZipUtility.ExtractFromStreamingAssets (view.cb)
			);
		} else {
