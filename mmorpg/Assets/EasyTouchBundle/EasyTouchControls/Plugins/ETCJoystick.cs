...
public class ETCJoystick : ETCBase,IPointerEnterHandler,IDragHandler, IBeginDragHandler, IPointerDownHandler, IPointerUpHandler {
	...
	void Start(){
		...
		float offset = 10;
		LeftMinPos=new Vector2(-canvasRectTransform.sizeDelta.x*0.5f+GetJostickLeft(offset),-canvasRectTransform.sizeDelta.y*0.5f+GetJostickLeft(offset));
...
//				Debug.Log("坐标："+localPosition);
				...
				if (overGO == null || (allowJoystickOverTouchPad && overGO.GetComponent<ETCTouchPad>()) || (overGO != null && overGO.GetComponent<ETCArea>() ) ) {
					if(localPosition.x < LeftMinPos.x)
						localPosition=new Vector2(LeftMinPos.x,localPosition.y);
					if(localPosition.y < LeftMinPos.y)
						localPosition=new Vector2(localPosition.x,LeftMinPos.y);
	...
	private void SetJoystickPosition(){
		float offset = 60;
		cachedRectTransform.anchoredPosition = new Vector2(GetJostickLeft(offset)-canvasRectTransform.sizeDelta.x*0.5f,
			GetJoystickDown(offset)-canvasRectTransform.sizeDelta.y*0.55f);
	}

	private float GetJoystickDown(float offset){
		return GetRadius() + offset;
	}

	private float GetJostickLeft(float offset){
		return GetRadius() + offset;
	}
