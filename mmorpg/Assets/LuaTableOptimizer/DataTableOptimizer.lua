...
If you want to exlucde some input files in DatabaseRoot, just add theirs names into IgnoreFile



--]]

function gf_localize_string(name)
	return name
end

local Root = arg[1]
-- local Root = "/Volumes/SevenSSD/U3dProject/client/client-android/Assets/LuaTableOptimizer"

Root = string.gsub( Root, '\\', '/' )
local DatabaseRoot = Root.."/Database"
print("DatabaseRoot=",DatabaseRoot)
local LuaRoot = Root.."/LuaRoot"
...
local DatabaseLocaleTextName = "local_text"
...
local LocaleTextLeadingTag = 'gf_localize_string'
local MaxLocalVariableNum = 180 -- lparser.c #define MAXVARS 200
local RefTableName = "__rt"
local DefaultValueTableName = "__default_values"
local EnableDatasetOptimize = true
local EnableDefaultValueOptimize = false -- 是否导出DefaultValue/true为导出
...
local function _SerializeTable( val, name, skipnewlines, campact, depth, tableRef )
    ...
    elseif valt == "string" then
	    if string.find(val, LocaleTextLeadingTag) then
	        append( val )
	    else
...
local function GetAllFileNamesAtPath( path )
	...
	for dir in io.popen( string.format( "find * %s", path ) ):lines() do
		
		if not dir:find("Config/") and not dir:find("LuaRoot/") then
			local s, e, f = dir:find( ".+/(.+)%.lua$" )

			if f and not IgnoreFile[f] then
				table.insert( ret, f )
			end

			local s, e, f = dir:find( ".+/(.+)%.lua.meta$" )
			if f then
				os.remove(dir)
			end
		end
...
local function LoadDataset( name )
	...
	local loader = function( name )
		...
		local rval = chunk() or {}
		print("fileName",fileName,rval,chunk)
...
local function CheckNotAscii( v )
	if v ~= nil and type( v ) == "string" then
		if string.find(v,LocaleTextLeadingTag) then
			return false
		end
		...
		function( k, v )
			...
			if vt == "string" then
				if CheckNotAscii( v ) then
					...
					localized_fields[ k ] = string.format( "%s(%s%s%s)", LocaleTextLeadingTag, "\"",v, "\"")
					if genCode then
						genCode[ #genCode + 1 ] = {
							id,
							sid,
...
do
    ...
    local pairs_func = function (t, key)
        local nk, nv = next(%s, key)
        if nk then 
            nv = t[nk]
        end
        return nk, nv
    ...
    --重写 pairs
    mt.__pairs = function (t, key)
       return pairs_func, t, nil
    end

    local ipairs_func = function(t,k)
        k = k+1
        if t[k] then
            return k,t[k]
        end
    ...
    mt.__metatable = false
						...
						]],DefaultValueTableName,DefaultValueTableName,datasetName)
					)
				end
			end
		}
