...
namespace Seven.Utils
{
	public static class ZipUtility
	{
		...
		public abstract class UnzipCallback
		{
			...
			/// </summary>
			/// <param name="_entry"></param>
			public virtual void OnPostUnzip(long totalSize, ZipEntry entry) {}
		...
		public static IEnumerator ExtractFromStreamingAssets(UnzipCallback cb)
		{  
...
//			Debug.Log ("src:" + src);  
			...
			if (!string.IsNullOrEmpty (www.error)) {  
				...
				Thread thred = new Thread(new ThreadStart(delegate {
					UnzipFile (stream, CUtils.uncompressStreamingAssetsPath, null, cb);
				}));
				thred.Start();
			}  
					...
					try
					{
						using (FileStream fileStream = File.Create(filePathName))
						{
							...
							while (true)
							{
								...
								{
									if (null != _unzipCallback)
										_unzipCallback.OnPostUnzip(_inputStream.Length,entry);
