...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
		...
		public LoopItem GetItem(int index)//获取一个item，下标从1开始
		{
			return content.GetChild (index).GetComponent<LoopItem> ();
		}
...
//                        dataSource.ProvideData(content.GetChild(i), itemTypeEnd);
						if (onItemRender != null) {
							LoopItem item = content.GetChild(i).gameObject.GetComponent<LoopItem> ();
							item.index = itemTypeEnd + 1;
							onItemRender.call (item, itemTypeEnd+1);
						}
                        itemTypeEnd++;
                    }
                    else
        ...
        public void RefillCells(int offset = 0)
        {
			...
			if (offset > 0) {//这里主要是为了和luatable下标一样
				offset--;
			}
        ...
        {        
			...
			if (onItemRender != null) {
				LoopItem item = nextItem.gameObject.GetComponent<LoopItem> ();
				item.index = itemIdx + 1;
				onItemRender.call (item, itemIdx+1);
			}
