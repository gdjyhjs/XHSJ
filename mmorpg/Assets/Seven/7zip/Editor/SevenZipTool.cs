...
public class CompressAndDecompress : Editor {  
	...
	{  
		string path = EditorUtility.OpenFilePanel("Compress file", "", "u3d");  
		CompressFile(path);  
	}  
	...
	// 压缩文件  
	[MenuItem("Compress/Compress File")]
	...
	{  
		...
		{  
			...
			{  
				...
				if (isCompress)  
				{  
					if (file.Extension == ".u3d" || file.Extension == ".lua" || file.Extension == ".manifest")  
					{  
				...
				{  
					...
					{  
						string outputPath = file.FullName.Replace(file.Extension, ".u3d");  
