...
python $PROJECT_ROOT/tools/autobuild/scripts/commit.py update $SVN_PATH/android
rm -rf $SVN_PATH/android
cp -rf $PACKAGE_PATH/FirstPackage/release/android $SVN_PATH
rm -rf $UPDATE_PATH/android
cp -rf $PACKAGE_PATH/FirstPackage/release/android $UPDATE_PATH
chown -R Seven $SVN_PATH/android
chown -R Seven $UPDATE_PATH/android
python $PROJECT_ROOT/tools/autobuild/scripts/commit.py commit $SVN_PATH/android
