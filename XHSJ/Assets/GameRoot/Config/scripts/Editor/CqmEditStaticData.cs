...
/// 读取静态配置表的基础编辑器类
/// </summary>
abstract public class CqmEditStaticData : Editor
{
    ...
    abstract public bool CheckTable(CqmStaticDataCenter dataCenter);
        ...
        if (GUILayout.Button("Load Excel"))
        {
            if (EditorUtility.DisplayDialog("读取配置表", "确定要导入这个配置表吗 ?   ", "OK", "Cancel"))
            {
                if (LoadXlsx())
                {
                    ...
                    CqmStaticDataCenter dataCenter = AssetDatabase.LoadAssetAtPath<CqmStaticDataCenter>(CqmStaticDataCenter.prefabPath);
...
public class CqmEditTemplateStaticData<T> : CqmEditStaticData where T : cqmStaticDataTableBase
{
    protected T script;
    ...
    public override bool CheckTable(CqmStaticDataCenter dataCenter)
    {
...
/// 解析表格数据的基类，提供接口用
/// </summary>
public class CqmEditStaticDataParser<TK>
{
    //读取一个字段的接口
    public virtual bool OnRow(cqmStaticIDData<TK> data, string key, string value)
    {
        return false;
    }
...
/// </summary>
/// <typeparam name="T"></typeparam>
public class CqmEditStaticDataTemplateParser<T, TK> : CqmEditStaticDataParser<TK> where T : CqmStaticData
{
    ...
    public override bool OnRow(cqmStaticIDData<TK> data, string key, string value)
    {
...
//[CustomEditor(typeof(cqmStaticIDDataTable<T>))]
public class CqmEditIDTemplateStaticData<T, TK> : CqmEditTemplateStaticData<cqmStaticIDDataTable<T, TK>> where T : cqmStaticIDData<TK>, new()
{
    ...
    public override bool OnFetchData(DataTable dataTable)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            //依次用解析器
            foreach (CqmEditStaticDataParser<TK> parser in parsers)
            {
    ...
    /// 解析器
    /// </summary>
    private List<CqmEditStaticDataParser<TK>> parsers = new List<CqmEditStaticDataParser<TK>>();

    protected void AddParser<TP>() where TP:CqmEditStaticDataParser<TK>, new()
    {
        TP p = new TP();
        p.Init();
        parsers.Add(p);
...
public class CqmEditIntIDTemplateStaticData<T> : CqmEditIDTemplateStaticData<T, int> where T : cqmStaticIDData<int>, new()
{

    //定义读取ID的解析类
    public class Parser : CqmEditStaticDataTemplateParser<cqmStaticIDData<int>, int>
    {
...
public class CqmEditStringIDTemplateStaticData<T> : CqmEditIDTemplateStaticData<T, string> where T : cqmStaticIDData<string>, new()
{

    //定义读取ID的解析类
    public class Parser : CqmEditStaticDataTemplateParser<cqmStaticIDData<string>, string>
    {
        public override void Init()
        {
            base.Init();
